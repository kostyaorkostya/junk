...
fn parse_field(data: &[u8]) -> (Field, &[u8]) {
    ...
    let (value, remainder) = match wire_type {
        WireType::Varint => {
            let (x, remainder) = parse_varint(remainder);
            (FieldValue::Varint(x), remainder)
        }
        WireType::Len => {
            let (len, remainder) = parse_varint(remainder);
            let len: usize = len.try_into().expect("Not a valid usize");
            let Some((data, remainder)) = remainder.split_at_checked(len) else {
